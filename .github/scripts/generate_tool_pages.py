...
def build_fallback_story(t, is_free: bool) -> str:
    ...
    impact_html = f'<h3>The Payoff</h3><p>{esc(impact_lede)}</p><p>{esc(impact_body)}</p>'
    ...
    verdict_html = f'<h3>The Verdict</h3><p>{esc(verdict_text)}</p>'
...
def generate_story_with_claude(t, is_free: bool):
    ...
    prompt = (
        ...
        f"1. <h3>The Payoff</h3> — open with a specific, vivid scene of the exact "
        ...
        f"3. <h3>The Verdict</h3> — an honest, specific verdict using the real "
